...
def _traverse(graph, start, pop_index):
    visited = set()
    pending = [start]

    while pending:
        node = pending.pop(pop_index)
        if node in visited:
            continue

        visited.add(node)
        pending.extend(neighbor for neighbor in graph[node] if neighbor not in visited)

    return visited


def dfs_traverse(graph, start):
    return _traverse(graph, start, -1)


def bfs_traverse(graph, start):
    return _traverse(graph, start, 0)
