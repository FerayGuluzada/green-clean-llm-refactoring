...
class SqrtDecomposition:
    ...
    def _validate_index(self, index: int) -> None:
        """Validate if the index is within the bounds of the array."""
    ...
    def _query_block(self, block: int) -> int | float:
        """Return the sum of elements in the block."""
        return self.blocks[block]

    def _query_range(self, left: int, right: int) -> int | float:
    ...
    def query(self, left: int, right: int) -> int | float:
        ...
        if block_left == block_right:
            ...
            return self._query_range(left, right)
        else:
            # Partial left block
            total += self._query_range(left, (block_left + 1) * self.block_size - 1)
            # Full middle blocks
            for b in range(block_left + 1, block_right):
                total += self._query_block(b)
            # Partial right block
            total += self._query_range(block_right * self.block_size, right)
            return total
