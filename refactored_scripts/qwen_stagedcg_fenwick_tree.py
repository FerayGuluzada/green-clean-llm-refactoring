...
class Fenwick_Tree(object):
    ...
    def get_sum(self, bit_tree, i):
        ...
        while i > 0:
            s += bit_tree[i]  # Add current element to sum
    ...
    def update_bit(self, bit_tree, i, v):
        ...
        The given value 'v' is added to bit_tree[i] and all of its ancestors.
        """
        i += 1  # index in bit_tree[] is 1 more than the index in arr[]
    ...
    def construct(self):
        ...
        # Store the actual values using update()
...
if __name__ == "__main__":
    ...
    arr_len = len(arr)
    updates = [i % 3 + 1 for i in range(arr_len)]
    
    for _ in range(20):
        # Batch updates for better cache locality
        for i in range(arr_len):
            ft.update_bit(bit_tree, i, updates[i])
        
        # Batch queries for better cache locality
        for i in range(arr_len):
