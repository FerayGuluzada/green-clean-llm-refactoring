...
class KDTree:
    ...
    def _build(self, points: list[tuple[float, ...]], depth: int) -> KDNode | None:
        ...
        # Use nth_element approach: find median without full sort
    ...
    def _nearest(
        ...
        # Precompute squared distance once
        dist = _sq_dist_fast(node.point, target)
        if dist < best[1]:
        ...
        # Determine close and away branches
        if diff <= 0:
            close, away = node.left, node.right
        else:
            close, away = node.right, node.left
        
        # Search close branch first
        self._nearest(close, target, best)
        
        # Only search away branch if it could contain a closer point
        if diff * diff < best[1]:
            self._nearest(away, target, best)


def _sq_dist_fast(a: tuple[float, ...], b: tuple[float, ...]) -> float:
