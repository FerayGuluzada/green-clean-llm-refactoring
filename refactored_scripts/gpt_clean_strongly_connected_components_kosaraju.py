...
class Kosaraju:
    ...
    def _dfs_fill_order(self, node, adj, visited, stack):
        visited[node] = True
        for neighbor in adj[node]:
            if not visited[neighbor]:
                self._dfs_fill_order(neighbor, adj, visited, stack)
        stack.append(node)
    ...
    def kosaraju(self, V, adj):
        ...
        for node in range(V):
            if not visited[node]:
                self._dfs_fill_order(node, adj, visited, stack)
        ...
        while stack:
            ...
            if not visited[node]:
                ...
                self._dfs_mark_component(node, transposed_adj, visited)
