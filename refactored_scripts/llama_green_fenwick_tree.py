...
class Fenwick_Tree(object):
    ...
    def bulk_update(self, bit_tree, updates):
        """
        Updates multiple nodes in the Binary Indexed Tree.
        """
        for i, v in updates:
            self.update_bit(bit_tree, i, v)

    def bulk_get_sum(self, bit_tree, indices):
        """
        Returns the sum of the first i elements for multiple indices.
        """
        return [self.get_sum(bit_tree, i) for i in indices]
...
if __name__ == "__main__":
    ...
    for _ in range(20):
        updates = [(i, i % 3 + 1) for i in range(len(arr))]
        ft.bulk_update(bit_tree, updates)
        total += sum(ft.bulk_get_sum(bit_tree, range(len(arr))))
