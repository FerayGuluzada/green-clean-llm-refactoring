...
def longest_increasing_subsequence_optimized(sequence: list[int]) -> int:
    ...
    def update(pos: int, left: int, right: int, target: int, value: int) -> None:
        if left == right:
            tree[pos] = value
            return
        mid = (left + right) >> 1
        if target <= mid:
            update(pos << 1, left, mid, target, value)
        else:
            update((pos << 1) | 1, mid + 1, right, target, value)
        tree[pos] = max(tree[pos << 1], tree[(pos << 1) | 1])

    def get_max(pos: int, left: int, right: int, start: int, end: int) -> int:
        ...
        mid = (left + right) >> 1
        return max(
            get_max(pos << 1, left, mid, start, end),
            get_max((pos << 1) | 1, mid + 1, right, start, end),
        )

    ans = 0
    for element in sequence:
        cur = get_max(1, 0, max_val, 0, element - 1) + 1
        ans = max(ans, cur)
        update(1, 0, max_val, element, cur)
    return ans
