...
class PriorityQueue:
    ...
    def __init__(
        ...
        if priorities is None:
            pairs = [(item, item) for item in items]
        else:
            pairs = [
                (item, item if priority is None else priority)
    ...
    def push(self, item: Any, priority: Any = None) -> None:
        ...
        index = bisect_left(priorities, insert_priority)
        priorities.insert(index, insert_priority)
        self.priority_queue_list.insert(index, PriorityQueueNode(item, priority))
