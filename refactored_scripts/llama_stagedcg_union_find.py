...
class Union:
    ...
    def _find_root(self, element: object) -> object:
        ...
        """
        parent = self._data[element]["parent"]
        if parent!= element:
            # Path compression: update parent of element to its root
            self._data[element]["parent"] = self._find_root(parent)
    ...
    def unite(self, element1: object, element2: object) -> None:
        ...
        if self._data[root1]["size"] <= self._data[root2]["size"]:
            ...
            self._data[root2]["parent"] = root1
            self._data[root1]["size"] += self._data[root2]["size"]
