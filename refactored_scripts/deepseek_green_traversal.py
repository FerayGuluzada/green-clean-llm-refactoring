...
def dfs_traverse(graph, start):
    ...
    while stack:
        ...
        if node not in visited:
            visited.add(node)
            # Add unvisited neighbors in reverse order to maintain original DFS order
            # while avoiding repeated visited checks in the stack
            for nextNode in reversed(list(graph[node])):
...
def bfs_traverse(graph, start):
    visited = set([start])
    queue = [start]
    front = 0  # Use index pointer instead of pop(0) for O(1) deque operations
