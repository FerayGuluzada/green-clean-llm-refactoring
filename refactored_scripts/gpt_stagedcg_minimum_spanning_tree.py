...
class DisjointSet:
    ...
    def mergeSet(self, root_a, root_b):
        # Args:
        #   root_a, root_b (int): Roots of nodes whose sets will be merged.
        ...
        parent[root_b] = root_a  # Merge set(b) into set(a)
        size[root_a] += size[root_b]  # Add size of old set(b) to set(a)
        return True
    ...
    def findSet(self, a):
        ...
        # Very important, memoize result in the list to optimize next
...
def kruskal(n, edges, ds):
    ...
    selected_edges = 0
    find_set = ds.findSet
    merge_set = ds.mergeSet
    target_edges = n - 1

    for edge in edges:
        root_u = find_set(edge.u)
        root_v = find_set(edge.v)

        if root_u == root_v:
            continue

        merge_set(root_u, root_v)
        mst_weight += edge.weight
        selected_edges += 1

        if selected_edges == target_edges:
