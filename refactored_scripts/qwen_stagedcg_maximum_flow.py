...
def ford_fulkerson(capacity, source, sink):
    ...
    # Use list of lists instead of nested list comprehension for better performance
...
def edmonds_karp(capacity, source, sink):
    ...
    # Pre-allocate parent array outside loop to reduce allocations
    parent = [-1] * vertices
...
def dinic(capacity, source, sink):
    ...
    # Pre-allocate arrays outside the main loop
    level = [-1] * vertices
    work = [0] * vertices
    ...
    while True:
        ...
        # Reset work array for each blocking flow phase
        for i in range(vertices):
            work[i] = 0
