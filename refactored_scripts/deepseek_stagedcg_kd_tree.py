...
class KDTree:
    ...
    def _build(self, points: list[tuple[float, ...]], depth: int) -> KDNode | None:
        ...
        axis = depth % self.k
        # Use faster sorting with precomputed key
    ...
    def _nearest(
        ...
        if node.point == target:
            return _BestCandidate(node.point, 0.0)
        ...
        # Compare squared distance to avoid sqrt computation
...
def _sq_dist(a: tuple[float, ...], b: tuple[float, ...]) -> float:
    ...
    # Use manual loop instead of zip+sum for better performance
    s = 0.0
