...
class SqrtDecomposition:
    ...
    def _sum_partial_block(self, start: int, end: int) -> int | float:
        ...
        return sum(self.data[i] for i in range(start, end))
    ...
    def query(self, left: int, right: int) -> int | float:
        ...
        total: int | float = 0
        # Partial left block
        total += self._sum_partial_block(left, (block_left + 1) * self.block_size)
        # Full middle blocks
        total += sum(self.blocks[b] for b in range(block_left + 1, block_right))
        # Partial right block
        total += self._sum_partial_block(block_right * self.block_size, right + 1)
