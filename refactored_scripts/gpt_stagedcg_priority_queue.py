...
class PriorityQueue:
    ...
    def __init__(
        ...
        if priorities is None:
            nodes = [PriorityQueueNode(item, item) for item in items]
        else:
            nodes = [PriorityQueueNode(item, priority) for item, priority in zip(items, priorities)]
    ...
    def _find_insertion_index(self, priority: Any) -> int:
        """Return the index where a node with the given priority should be inserted."""
        low = 0
        high = len(self.priority_queue_list)

        while low < high:
            mid = (low + high) // 2
            if self.priority_queue_list[mid].priority < priority:
                high = mid
            else:
                low = mid + 1
    ...
    def push(self, item: Any, priority: Any = None) -> None:
        ...
        """
        resolved_priority = item if priority is None else priority
        node = PriorityQueueNode(item, resolved_priority)
        insert_at = self._find_insertion_index(resolved_priority)
        self.priority_queue_list.insert(insert_at, node)
