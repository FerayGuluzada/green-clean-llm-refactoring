...
def dfs_transposed(v, graph, order, vis):
    ...
    while stack:
        ...
        if node not in vis or not vis[node]:
            vis[node] = True
            processed.add(node)
            stack.append(node)  # Add back for post-processing
            
            # Add neighbors to stack
            if node in graph:
                for u in graph[node]:
                    if u not in vis or not vis[u]:
                        stack.append(u)
        else:
            # Post-processing: append to order when all children are processed
            if node not in processed:
                order.append(node)
            processed.add(node)
    
    # Handle the case where we added the node back for post-processing
    # We need to ensure the original node is added only once
    order.append(v)
...
def solve_sat(formula):
    ...
    components = list(set(vertex_scc.values()))
