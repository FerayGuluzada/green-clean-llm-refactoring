...
def _helper_topdown(
    ...
    if dp[target]!= -1:
        return dp[target]
    # Limit the recursive calls to only valid numbers
    result = sum(
        _helper_topdown(nums, target - num, dp) 
        for num in nums 
        if target >= num
    )
    dp[target] = result
...
def combination_sum_topdown(nums: list[int], target: int) -> int:
    ...
    Examples:
        ...
        7
    """
    # Initialize dp table with a size of target + 1
    dp = [-1] * (target + 1)
    # Base case: there is one way to sum to 0 (by not picking any numbers)
    dp[0] = 1
...
def combination_sum_bottom_up(nums: list[int], target: int) -> int:
    ...
    for i in range(1, target + 1):
        # Limit the inner loop to only valid numbers
        for num in nums:
            if i - num >= 0:
                combs[i] += combs[i - num]
    return combs[target]
