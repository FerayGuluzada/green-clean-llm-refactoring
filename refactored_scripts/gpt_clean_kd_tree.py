...
class KDTree:
    ...
    def nearest(self, target: tuple[float, ...]) -> tuple[float, ...]:
        ...
        best_point, _ = self._nearest(self.root, target, None, math.inf)
        return best_point
    ...
    def _nearest(
        ...
        node_dist = _sq_dist(node.point, target)
        if node_dist < best_dist:
            best_point, best_dist = node.point, node_dist

        axis = node.axis
        diff = target[axis] - node.point[axis]
        near_branch, far_branch = self._branches(node, diff)

        best_point, best_dist = self._nearest(near_branch, target, best_point, best_dist)
        if diff * diff < best_dist:
            best_point, best_dist = self._nearest(
                far_branch, target, best_point, best_dist
            )

        return best_point, best_dist

    @staticmethod
    def _branches(node: KDNode, diff: float) -> tuple[KDNode | None, KDNode | None]:
        if diff <= 0:
