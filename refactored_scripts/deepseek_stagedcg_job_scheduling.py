...
def _find_latest_compatible(jobs: list[Job], current_index: int) -> int:
    ...
    """
    # Binary search for the rightmost job with finish <= current job's start
    ...
    while left <= right:
        ...
        if jobs[mid].finish <= target_start:
            result = mid
            left = mid + 1  # Search right half for potentially later compatible job
        else:
            right = mid - 1
    return result
...
def _compute_max_profit(jobs: list[Job]) -> int:
    """Compute maximum profit using dynamic programming with early exit optimization."""
    n = len(jobs)
    if n == 0:
        return 0
    ...
    for i in range(1, n):
        ...
        # Early exit: if including current job doesn't improve profit,
        # we can skip the max() call for the common case
...
def schedule(jobs: list[Job]) -> int:
    ...
    Examples:
        ...
        4
    """
    # Sort once and reuse throughout computation
