...
if __name__ == "__main__":
    ...
    # Cache len(arr) to avoid repeated function calls
    arr_len = len(arr)
    
    # Optimize the nested loops by reducing function call overhead
    for _ in range(20):
        # Inline the first loop with direct calculation
        for i in range(arr_len):
            # Directly access bit_tree since we know the structure
            j = i + 1  # 1-indexed position
            v = i % 3 + 1
            while j <= arr_len:
                bit_tree[j] += v
                j += j & (-j)
        
        # Inline the second loop with direct calculation
        for i in range(arr_len):
            s = 0
            j = i + 1  # 1-indexed position
            while j > 0:
