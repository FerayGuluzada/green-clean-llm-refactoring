...
class DisjointSet:
    def __init__(self, n):
        ...
        # Contains wich node is the parent of the node at poisition <i>
    ...
    def mergeSet(self, a, b):
        ...
        if size[a] < size[b]:
            parent[a] = b  # Merge set(a) and set(b)
            size[b] += size[a]  # Add size of old set(a) to set(b)
        else:
            parent[b] = a  # Merge set(b) and set(a)
            size[a] += size[b]  # Add size of old set(b) to set(a)
...
def kruskal(n, edges, ds):
    ...
    for edge in edges:
        set_u = find_set(edge.u)  # Set of the node <u>
        set_v = find_set(edge.v)  # Set of the node <v>
        if set_u != set_v:
            # Join the shortest node to the longest, minimizing tree size (faster find)
