...
class Union:
    ...
    def root(self, element: object) -> object:
        ...
        """
        if element not in self.parents:
            raise KeyError(f"Element {element} not found in Union-Find structure")
        
        # Path compression - iterative to avoid recursion overhead
        root = element
        ...
        # Compress path
        while element != root:
            next_element = self.parents[element]
    ...
    def unite(self, element1: object, element2: object) -> None:
        ...
        """
        # Direct access to roots with path compression
        root1 = element1
        while root1 != self.parents[root1]:
            root1 = self.parents[root1]
            
        root2 = element2
        while root2 != self.parents[root2]:
            root2 = self.parents[root2]
        
        if root1 == root2:
            return
            
        # Union by size: ensure smaller tree is merged into larger tree
